...
export function Step2({
  ...
  const {
    ...
    queryKey: ["getStudentsByGradeAndSection"],
    queryFn: () =>
      getStudentsByGradeAndSection({
  ...
  useEffect(() => {
    ...
    if (!isValid) {
      if (remainingLicenses.annual < 0 && remainingLicenses.monthly < 0) {
        setErrorMessage(
          `License limit exceeded: You need ${Math.abs(
            remainingLicenses.annual
          )} more annual and ${Math.abs(
            remainingLicenses.monthly
          )} more monthly licenses.`
        );
      } else if (remainingLicenses.annual < 0) {
        setErrorMessage(
          `Annual license limit exceeded: You need ${Math.abs(
            remainingLicenses.annual
          )} more annual license${
            Math.abs(remainingLicenses.annual) > 1 ? "s" : ""
          }.${
            remainingLicenses.monthly > 0
              ? ` You have ${remainingLicenses.monthly} monthly license${
                  remainingLicenses.monthly > 1 ? "s" : ""
                } available.`
              : ""
          }`
        );
      } else if (remainingLicenses.monthly < 0) {
        setErrorMessage(
          `Monthly license limit exceeded: You need ${Math.abs(
            remainingLicenses.monthly
          )} more monthly license${
            Math.abs(remainingLicenses.monthly) > 1 ? "s" : ""
          }.${
            remainingLicenses.annual > 0
              ? ` You have ${remainingLicenses.annual} annual license${
                  remainingLicenses.annual > 1 ? "s" : ""
                } available.`
              : ""
          }`
        );
  ...
  const availableStudentsByGrade = useMemo(() => {
    ...
    return studentsByGradeAndSectionData.filter(
      (student: any) => !selectedStudents.some((s: any) => s.id === student.id)
    );
  }, [studentsByGradeAndSectionData, selectedStudents]);

  console.log({ availableStudentsByGrade });
  ...
  const updateLicenseType = (
    ...
    setSelectedStudents((prev: any) =>
      prev.map((s: any) =>
        s.studentId === studentId ? { ...s, licenseType: newLicenseType } : s
      )
    );
    setErrorMessage("");
  };
  const validateAndAddStudents = () => {
    ...
    const newStudents = selectedToAdd
      .map((student) => {
        ...
        return null; // This should never happen due to validation
      })
      .filter(Boolean);
